...
from MCTS.runMCTS import MonteCarloTreeSearch
#===========================================================
...
#===============================================================
#Funciton Name: reciveJsonFromClient (Monte Carlo part)
#Description: recive the post data which content the box_info and container_info
#Function: Send data to backend RL to run testing with the given info
#Return: data send back to client to render 3d
#===============================================================
@app.route('/api/montecarlo', methods=["POST"])
def recieveJsonFromClient_MC():
    ...
    data = processDataCustom(container_infos, box_infos, pallet_infos)

    print("+++++++++++++++++++++++++++++++")
    print(data)

    # ********* pallet_infos *********   
    # for i in pallet_infos:
    #     print(i)
    ...
    rjson = MonteCarloTreeSearch(data)

    return rjson


#===============================================================
#Funciton Name: reciveJsonFromClient (RL part)
#Description: recive the post data which content the box_info and container_info
#Function: Send data to backend RL to run testing with the given info
#Return: data send back to client to render 3d
#===============================================================
@app.route('/api/rltesting', methods=["POST"])
def recieveJsonFromClient_RL():
    info_jsondata = request.get_json(force=True)
    ...
    print("this is container info")
    print("***************************")
    print(container_infos)

    print("this is box info")
    print("***************************")
    print(box_infos)
    
    print("this is pallet info")
    print("***************************")
    print(pallet_infos)

    #process to feed rl model
    data = processDataCustom(container_infos, box_infos, pallet_infos)

    print(data)

    return data
    #set config for rl training
    setConfig(data)
    ...
    #turn data into 3D render-able data
    out = processDatato3D(out)

    return out

def setConfig(data):
    config.container_size = data["container"]["size"]
    config.box_size_set = [b["size"] for b in data["box"]]
    config.pallet_size = [p["size"] for p in data["pallet"]]

def processDatato3D(out):
    return 0

def processDataCustom(c_infos, b_infos, p_infos):
    dic = {}
    
    for c_info in c_infos:
        dic["container"] = {
                "ID": c_info["ID"],
                "TypeName": c_info["TypeName"],
                "size": (int(c_info["X"]), int(c_info["Y"]), int(c_info["Z"])),
                "weight_limit":c_info["Weight_limmit"],
            }
    
    dic["total_container_types"] = len(c_infos)

    # sort box by volume
    b_infos=sorted(b_infos,  key=lambda box_info: int(box_info["X"])* int(box_info["Z"])*int(box_info['Y']), reverse=True)
    print(b_infos)

    dic["box"] = []
    # make input sequence
    for b_info in b_infos:
        for _ in range(int(b_info["Numbers"])):
            dic["box"].append(
                {
                    "ID": b_info["ID"],
                    "TypeName": b_info["TypeName"],
                    "size": (int(b_info["X"]), int(b_info["Y"]), int(b_info["Z"])),
                    "Weight": b_info["Weight"]
                }
            )
    
    dic["total_box_types"] = len(b_infos)

    dic["pallet"] = []
    # define pallets as small containers
    for p_info in p_infos:
        for _ in range(int(p_info["Numbers"])):
            dic["pallet"].append(
                {
                    "ID": p_info["ID"],
                    "TypeName": p_info["TypeName"],
                    "size":(int(p_info["X"]), int(c_infos[0]["Y"])-int(p_info["Y"]), int(p_info["Z"])),
                    "weight_limit": p_info["Weight"]
                }
            )

    dic["total_pallet_types"] = len(p_infos)
