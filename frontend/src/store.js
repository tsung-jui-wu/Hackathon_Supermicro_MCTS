...
const store = new Vuex.Store({
  ...
  mutations: {
    ...
    MUTATE_PALLET_INFO: (state, new_pallet_infos) => {
      state.pallet_infos = new_pallet_infos;
    },
    MUTATE_APPEND_CONTAINER_INFO(state, container_info_to_be_appended) {
      state.container_infos = [
        ...state.container_infos,
        ...container_info_to_be_appended,
      ];
    },
    ...
    MUTATE_DELETE_CONTAINER_WITH_UUID(state, uuid) {
      ...
      state.container_infos = state.container_infos.reduce(
        (new_array, current_val) => {
    ...
    MUTATE_DELETE_PALLET_WITH_UUID(state, uuid) {
      state.pallet_infos = state.pallet_infos.reduce(
        (new_array, current_val) => {
          current_val.ID !== uuid && new_array.push(current_val);
          return new_array;
        },
  ...
  actions: {
    ...
    loadPalletInfos: (context, pallet_infos) => {
      context.commit("MUTATE_PALLET_INFO", pallet_infos);
    },
  ...
  getters: {
    containerInfos: (state) => state.container_infos,
    palletInfos: (state) => state.pallet_infos,
    boxInfos: (state) => state.box_infos,
  },
}); //end store
